...
public class AI_1 : ShotAI
{
    ...
    void Start()
    {
        ...
        workPorce = 10;
        weaponName = "단발 권총";
        level = SaveScript.hasAI[0].level;
        ...
        ShoutDelayTime = 0.5f;
        ReloadTime = 1.666666f;
        initAngle = 0f;
        range = CameraCtrl.cameraRadius * 1.1f;
        isFront = 1;
        moveSpeed = 2.5f;
        backDis = range * 0.5f;

        effectColor = new Color(0.5f, 1, 0.5f, 0.6f); // 초록색
        color = new Color(0.5f, 1, 0.5f, 1f);
        abilities = new Ability[1];
        abilities[0] = new Ability(13);
        price = 500;
        redJam = 3;

        shop_image = AIObject.AIs_ShopImages[0];
        weapon_image = AIObject.AIs_WeaponImages[0];
    }
        ...
        if (SceneManager.GetActiveScene().name == "GameScene" && gameObject.name != "New Game Object")
        {
            ...
            if (!playerScript.isDead && playerScript.isStart && !SpecialAICtrl.isWork)
            {
                ...
                if (isEnemy && !isBack)
                {
                    if (isShout)
                    {
                        leftHandTr.transform.localPosition = new Vector3(Mathf.Abs(Vector2.ClampMagnitude(zoomVecSave, 1.1f).x), Vector2.ClampMagnitude(zoomVecSave, 1.1f).y)
                    ...
                    {
                        leftHandTr.transform.localPosition = new Vector3(Mathf.Abs(Vector2.ClampMagnitude(zoomVec, 1.1f).x), Vector2.ClampMagnitude(zoomVec, 1.1f).y)
                            - leftHandTr.transform.parent.transform.localPosition;
